...
public:
    ...
    bool ConnectToServer()
    {
        ...
        Print("[CONNECT] Connecting to dual WebSocket endpoints...");

        // 1. Connect main trading WebSocket
        if(!ConnectTradingWebSocket()) {
            Print("[ERROR] Failed to connect trading WebSocket");
            return false;
        }

        // 2. Connect price streaming WebSocket
        if(!ConnectPriceStreamWebSocket()) {
            Print("[ERROR] Failed to connect price stream WebSocket");
            m_isConnected = false;
            return false;
        }

        Print("[SUCCESS] Dual WebSocket connections established");
        Print("[INFO] Trading commands: " + m_serverUrl);
        Print("[INFO] Price streaming: " + m_priceStreamUrl);
    ...
    bool ConnectTradingWebSocket()
    {
        ...
        if(result && statusCode == 200) {
            m_isConnected = true;
            Print("[SUCCESS] Trading WebSocket connected");
            return true;
        } else {
            Print("[ERROR] Trading WebSocket failed. Status: " + IntegerToString(statusCode));
            m_connectionRetries++;
            return false;
        }
    }
    ...
    bool ConnectPriceStreamWebSocket()
    {
        ...
        if(result && (statusCode == 200 || statusCode == 404)) { // 404 is OK for price stream
            ...
            Print("[SUCCESS] Price stream WebSocket connected");
            return true;
        } else {
            Print("[ERROR] Price stream WebSocket failed. Status: " + IntegerToString(statusCode));
            return false;
        }
    }
    ...
    bool SendAccountProfile(string profileData)
    {
        ...
        if(result && (statusCode == 200 || statusCode == 201)) {
            Print("[SUCCESS] Account profile sent successfully");
            return true;
        } else {
            Print("[ERROR] Failed to send account profile. Status: " + IntegerToString(statusCode));
            if(statusCode == 401) {
                Print("[AUTH] Authentication failed - check JWT token");
            }
            return false;
        }
    }
    ...
    bool SendPriceData(string priceData)
    {
        ...
        if(result && statusCode == 200) {
            ...
            if(statusCode != 200) {
                Print("[WARNING] Price streaming error. Status: " + IntegerToString(statusCode));
    ...
    string GetServerCommands()
    {
        ...
        if(result && statusCode == 200) {
            ...
            Print("[WARNING] Failed to get trading commands. Status: " + IntegerToString(statusCode));
    ...
    void QueueCommands(string commandsJson)
    {
        ...
        if(m_commandCount < ArraySize(m_pendingCommands) - 1) {
            ...
            Print("[WARNING] Command queue full, dropping oldest command");
    ...
    bool SendTradeConfirmation(string confirmationData)
    {
        ...
        if(result && statusCode == 200) {
            Print("[SUCCESS] Trade confirmation sent");
            return true;
        } else {
            Print("[ERROR] Failed to send trade confirmation. Status: " + IntegerToString(statusCode));
            return false;
        }
    }
    ...
    bool IsHealthy()
    {
        ...
        if(!tradingHealthy) {
            Print("[WARNING] Trading connection unhealthy");
            m_isConnected = false;
        }

        if(!priceStreamHealthy) {
            Print("[WARNING] Price stream connection unhealthy");
            m_isPriceStreamConnected = false;
        }
        ...
        if(!tradingHealthy || !priceStreamHealthy) {
            if(!m_isReconnecting) {
                Print("[RECONNECT] Initiating auto-reconnection...");
    ...
    bool Reconnect()
    {
        Print("[RECONNECT] Forcing reconnection to server...");
        m_isConnected = false;
        m_isPriceStreamConnected = false;
        m_connectionRetries++;
...
private:
    ...
    bool HttpRequest(string method, string endpoint, string data, string &response, int &statusCode)
    {
        ...
        if(result == -1) {
            ...
            Print("[ERROR] HTTP request failed. Error: " + IntegerToString(error));
