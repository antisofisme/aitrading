...
class TransferManager extends EventEmitter {
    ...
    async send(data, destination, method = 'auto', options = {}) {
        ...
        try {
            ...
            // Try fallback if enabled and original method failed
            if (this.config.fallback_enabled && method !== 'http') {
                try {
                    console.log(`Attempting fallback to HTTP for ${destination}`);
                    const fallback_result = await this._executeWithRetries(
                        'http',
                        data,
                        destination,
                        options,
                    ...
                    console.error(`Fallback also failed: ${fallback_error.message}`);
                }
            }
    ...
    async _executeWithRetries(method, data, destination, options, correlation_id) {
        ...
        for (let attempt = 1; attempt <= this.config.retry_attempts; attempt++) {
            try {
                ...
                if (!adapter) {
                    throw new Error(`Transport adapter not available: ${method}`);
                }
    ...
    async _simulateTransport(method, data, destination, options) {
        // Simulate transport latency based on method
        const latencies = {
            'nats-kafka': 5 + Math.random() * 10,    // 5-15ms
            'grpc': 10 + Math.random() * 20,         // 10-30ms
            'http': 50 + Math.random() * 100        // 50-150ms
        };

        const latency = latencies[method] || 50;
        await this._sleep(latency);

        // Simulate occasional failures
        if (Math.random() < 0.02) { // 2% failure rate
            throw new Error(`Simulated ${method} transport failure`);
        }
        ...
        return {
            ...
            data_size: this._estimateDataSize(data),
            simulated: true
        };
    }
