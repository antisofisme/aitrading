...
async def register_service(registration: ServiceRegistration, request: Request) -> Dict[str, Any]:
    ...
    logger.info(f"🔍 register_service called for: {registration.name}")

    sys.path.insert(0, str(Path(__file__).parent.parent))
    from app import central_hub_service
    logger.info("✅ Imported central_hub_service")
    ...
    logger.info(f"📋 Registration data: name={registration_data['name']}, host={registration_data['host']}")

    # Process through contract validation if available
    validated_data = registration_data  # Default to original data
    transport_info = {}

    logger.info(f"📋 Initial validated_data set to registration_data")

    try:
        # Check if contract validation was already done by middleware
        if hasattr(request.state, 'contract_validation') and request.state.contract_validation:
            # Use pre-validated data from middleware
            validation_result = request.state.contract_validation
            if isinstance(validation_result, dict):
                validated_data = validation_result.get('validated_data') or registration_data
                transport_info = validation_result.get('transport_info', {})
            else:
                validated_data = registration_data
                transport_info = {}
            central_hub_service.logger.info(f"✅ Using middleware-validated data")
        else:
            ...
            try:
                ...
                validated_data = contract_result.get('validated_data') or registration_data
                transport_info = contract_result.get('transport_info', {})
                central_hub_service.logger.info(f"✅ Manual contract validation completed")
            except Exception as contract_error:
                central_hub_service.logger.warning(f"⚠️ Contract validation unavailable: {str(contract_error)}")
                # Use original data if validation fails
    except Exception as e:
        central_hub_service.logger.warning(f"⚠️ Validation error, using direct data: {str(e)}")
        validated_data = registration_data
        transport_info = {}

    # Ensure validated_data is never None
    if not validated_data:
        logger.warning("⚠️ validated_data is empty, using registration data")
        validated_data = registration_data
...
async def get_service(service_name: str, request: Request) -> Dict[str, Any]:
    ...
    service_info = central_hub_service.service_registry.services[service_name]
...
async def list_services() -> Dict[str, Any]:
    ...
    services = {}
    for name, info in central_hub_service.service_registry.services.items():
        services[name] = {
            "host": info["host"],
            "port": info["port"],
...
async def deregister_service(service_name: str) -> Dict[str, Any]:
    ...
    del central_hub_service.service_registry.services[service_name]
...
async def service_heartbeat_from_path(service_name: str) -> Dict[str, Any]:
    """Update service heartbeat (service_name in path)"""
    from app import central_hub_service
