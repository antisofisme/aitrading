#!/usr/bin/env python3
"""
Download Wheels Script - User Service  
...
def download_service_tier(tier_name):
    ...
    print(f"📦 Downloading {tier_name} for User service ({requirements_file})...")
    ...
    if run_command(cmd, f"Download {tier_name} wheels"):
        ...
        print(f"✅ User Service {tier_name} completed - Size: {size}")
        return True
    return False

def download_all_tiers():
    """Download all service tiers"""
    print("🚀 User Service - Complete Offline Wheels Download")
...
def main():
    parser = argparse.ArgumentParser(description="User Service Wheels Management")
    ...
    if args.tier:
        ...
        if success:
            print(f"\n🎯 User service ready for offline deployment!")
            print(f"   Next: docker-compose build user-service-{args.tier}")
        else:
