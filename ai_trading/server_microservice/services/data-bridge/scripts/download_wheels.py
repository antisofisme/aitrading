#!/usr/bin/env python3
"""
Download Wheels Script - Data Bridge Service  
...
def download_service_tier(tier_name):
    ...
    print(f"📦 Downloading {tier_name} for Data Bridge service ({requirements_file})...")
    ...
    if run_command(cmd, f"Download {tier_name} wheels"):
        ...
        print(f"✅ Data Bridge {tier_name} completed - Size: {size}")
        return True
    return False

def download_all_tiers():
    """Download all service tiers"""
    print("🚀 Data Bridge Service - Complete Offline Wheels Download")
...
def main():
    parser = argparse.ArgumentParser(description="Data Bridge Service Wheels Management")
    ...
    if args.tier:
        ...
        if success:
            print(f"\n🎯 Data Bridge service ready for offline deployment!")
            print(f"   Next: docker-compose build data-bridge-{args.tier}")
        else:
